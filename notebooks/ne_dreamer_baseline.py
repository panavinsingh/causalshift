"""NE-Dreamer baseline on CausalShift — Kaggle GPU notebook.

NE-Dreamer: reconstruction-free DreamerV3 variant with JEPA-style
...
- NE-Dreamer: encoder + latent predictor (predicts next latent, no decoder)
...
class NEDreamer:
    ...
    def train_world_model(self, data, steps=5000, batch_size=64):
        ...
        for step in range(steps):
            ...
            z = self.encoder(s)
            # Target computed inside loss (NE-Dreamer style)
            ...
            # NE-Dreamer: next embedding prediction with stop-gradient target
            with torch.no_grad():
                z_next_target = self.encoder(ns).detach()
            pred_loss = nn.functional.mse_loss(z_next_pred, z_next_target)
...
with open("ne_dreamer_xor_results.json", "w") as f:
    json.dump({"baseline": "ne_dreamer", "env": "xor", "summary": summary, "per_seed": all_results}, f, indent=2)

print("\n" + "=" * 60)
print("RESULTS: NE-Dreamer on XOR")
print("=" * 60)
for theta in SHIFT_LEVELS:
