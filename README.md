...
| Abstraction | Source (θ=0) | Shifted (θ=0.5) | Behavior |
|------------|-------------|-----------------|----------|
| φ_m (mechanism-invariant) | 0 regret | 0 regret | Transfer-robust |
| φ_p (predictive) | 0 regret | θ×T linear regret | Transfer-fragile |
| DBC, CBM, DreamerV3, etc. | 0 regret | ~θ×T linear regret | Learn φ_p |
| LLMs (privileged) | 100% accuracy | 100% accuracy | Act like φ_m |
| LLMs (blackbox) | ~50-100% | ~49-74% | Between φ_m and φ_p |

## Repository Structure

```
paper/
├── causalshift/                    # Core library
...
│   │   ├── mechanism_invariant.py  # φ_m: tracks causal variables
│   │   └── predictive.py           # φ_p: tracks effect variables
...
│   ├── full_separation.py          # Separation theorem validation
│   ├── xor_separation.py           # XOR-specific separation
│   ├── gpu_baselines.py            # DBC/CBM experiments
│   ├── llm_baselines.py            # LLM planner experiments
│   └── router_experiment.py        # Router ablation
├── notebooks/                      # GPU baseline notebooks (Lightning AI)
...
│   └── plot_all_results.py         # Figs 5-8: all baselines
├── results/                        # All experimental results (JSON)
│   ├── full_separation/            # Theorem validation (3 envs × 30 seeds)
│   ├── llm/                        # LLM baselines (4 models × 3 conditions)
│   ├── gpu/                        # RL baselines (6 methods × 30 seeds)
│   ├── router/                     # Router ablation (4 strategies × 30 seeds)
│   └── statistical_summary.json    # Aggregate statistics
├── figures/                        # Publication-ready figures (PDF + PNG)
│   ├── fig3_separation_three_panel.pdf
│   ├── fig4_gap_scaling.pdf
│   ├── fig5_llm_baselines.pdf
│   ├── fig6_gpu_baselines.pdf
│   ├── fig7_router_ablation.pdf
│   └── fig8_summary_heatmap.pdf
├── latex/                          # Paper source
│   ├── main.tex
│   └── references.bib
├── tests/                          # Unit tests
...
LLM experiments require API access to:
...
World model baselines (DreamerV3, Dreamer-CDP, TransDreamerV3, NE-Dreamer) require GPU access. Notebooks are designed to run on Lightning AI or any NVIDIA GPU.

## Citation

```bibtex
@inproceedings{singh2027cost,
  title={The Cost of Counterfactuals: A Separation Theorem for Transfer-Robust Abstraction in Modular Decision Processes},
  author={Singh, Panavin},
  booktitle={Proceedings of the 20th International Conference on Artificial General Intelligence (AGI-2027)},
  year={2027},
